{set $currentCity = $.session.'cityselector.current_city'}

{set $nfp = $_modx->config.ms2_price_format|json_decode}
{set $price = $is_group
    ? $group_price.$currentCity
    : $id | resource: $.session.'cityselector.current_product_price_tv'
}
{set $price = $price|number:0:$nfp.1:$nfp.2}

{set $remain = $is_group
    ? $group_remain.$currentCity
    : $id | resource : $.session.'cityselector.current_product_remain_tv'
}
